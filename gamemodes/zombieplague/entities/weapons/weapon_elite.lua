...
if ( CLIENT ) then       
	killicon.AddFont( "weapon_elite", "CSKillIcons", "s", Color( 255, 80, 0, 255 ) )
...
function SWEP:SetupDataTables()
	self:NetworkVar("Bool", 0, "LeftPistol")
...
function SWEP:PrimaryAttack()
	...
	if not self:GetLeftPistol() then
	if SERVER then
	self:SetLeftPistol(true)
	end
	elseif self:GetLeftPistol() then
	if SERVER then
	self:SetLeftPistol(false)
	end
	end

print(self:GetLeftPistol())
...
elseif self:GetLeftPistol() then
self:PlayAnim( ACT_VM_SECONDARYATTACK )
end
elseif self.Weapon:Clip1() == 2 then
self:PlayAnim( ACT_VM_DRYFIRE_LEFT )
